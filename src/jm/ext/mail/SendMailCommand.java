...
public class SendMailCommand {
    ...
    private static ThreadLocal<Transport> localTransport = new ThreadLocal<Transport>();
    private static ThreadLocal<SynchronousTransportListener> localListener = new ThreadLocal<SynchronousTransportListener>();
    ...
    public Message prepareMessage() throws MessagingException, IOException {
        ...
        // Extensions
        if(connectionTimeout > 0)  props.setProperty("mail.smtp.connectiontimeout", ""+connectionTimeout);
        if(socketTimeout > 0)   props.setProperty("mail.smtp.timeout", ""+socketTimeout);
        if (!quitWait) {
    ...
    public void reuseExecute(Message message) throws InterruptedException, MessagingException {
        Transport tr = localTransport.get();
        SynchronousTransportListener listener = localListener.get();
        if(tr == null) {
            tr = session.getTransport(getProtocol());
            logger.debug("transport created");
            localTransport.set(tr);
            if(this.synchronousMode == true) {
                //listener = localListener.get(); // Create listener immediately
                //if(listener == null) {           // Create listener immediately
                    listener = new SynchronousTransportListener();
                    tr.addTransportListener(listener);
    ...
    public void destroyTransport() throws MessagingException {
        localListener.get().finish();
        localListener.remove();
        localTransport.get().close();
        localTransport.remove();
    }
    public void reconnect() throws MessagingException {
        Transport tr = localTransport.get();
        if(tr == null) logger.error("can't find transport to reconnect");
        else {
            SynchronousTransportListener listener = localListener.get();
            localTransport.remove();
            localListener.remove();
