...
import jm.ext.mail.SendMailCommand;
 ...
 */
public class ExtSMTPSampler extends AbstractSampler {
    ...
    public static final String SERVER               = "ExtSMTPSampler.server"; // $NON-NLS-1$


    public static final String SERVER_PORT          = "ExtSMTPSampler.serverPort"; // $NON-NLS-1$
    public static final String USE_AUTH             = "ExtSMTPSampler.useAuth"; // $NON-NLS-1$
    public static final String USERNAME             = "ExtSMTPSampler.username"; // $NON-NLS-1$
    public static final String PASSWORD             = "ExtSMTPSampler.password"; // $NON-NLS-1$
    public static final String MAIL_FROM            = "ExtSMTPSampler.mailFrom"; // $NON-NLS-1$
    public static final String MAIL_REPLYTO         = "ExtSMTPSampler.replyTo"; // $NON-NLS-1$
    public static final String RECEIVER_TO          = "ExtSMTPSampler.receiverTo"; // $NON-NLS-1$
    public static final String RECEIVER_CC          = "ExtSMTPSampler.receiverCC"; // $NON-NLS-1$
    public static final String RECEIVER_BCC         = "ExtSMTPSampler.receiverBCC"; // $NON-NLS-1$

    public static final String SUBJECT              = "ExtSMTPSampler.subject"; // $NON-NLS-1$
    public static final String SUPPRESS_SUBJECT     = "ExtSMTPSampler.suppressSubject"; // $NON-NLS-1$
    public static final String MESSAGE              = "ExtSMTPSampler.message"; // $NON-NLS-1$
    public static final String PLAIN_BODY           = "ExtSMTPSampler.plainBody"; // $NON-NLS-1$
    public static final String INCLUDE_TIMESTAMP    = "ExtSMTPSampler.include_timestamp"; // $NON-NLS-1$
    public static final String ATTACH_FILE          = "ExtSMTPSampler.attachFile"; // $NON-NLS-1$
    public static final String MESSAGE_SIZE_STATS   = "ExtSMTPSampler.messageSizeStatistics"; // $NON-NLS-1$
    public static final String HEADER_FIELDS        = "ExtSMTPSampler.headerFields"; // $NON-NLS-1$

    public static final String USE_EML              = "ExtSMTPSampler.use_eml"; // $NON-NLS-1$
    public static final String EML_MESSAGE_TO_SEND  = "ExtSMTPSampler.emlMessageToSend"; // $NON-NLS-1$
    public static final String ENABLE_DEBUG         = "ExtSMTPSampler.enableDebug"; // $NON-NLS-1$
    
    //Extensions
    public static final String CONNECTION_TIMEOUT   = "ExtSMTPSampler.connectionTimeout"; // $NON-NLS-1$
    public static final String SOCKET_TIMEOUT       = "ExtSMTPSampler.socketTimeout"; // $NON-NLS-1$
    public static final String QUIT_WAIT            = "ExtSMTPSampler.quitWait"; // $NON-NLS-1$
    public static final String REUSE_TRANSPORT      = "ExtSMTPSampler.reuseTransport"; // $NON-NLS-1$
    public static final String RECONNECT            = "ExtSMTPSampler.reconnect"; // $NON-NLS-1$

    // Used to separate attachment file names in JMX fields - do not change!
    public static final String FILENAME_SEPARATOR = ";";
    //-JMX file attribute names
    
    public static ThreadLocal<SendMailCommand> localInstance = new ThreadLocal<SendMailCommand>();


    public ExtSMTPSampler() {
    }
    ...
    public SampleResult sample(Entry e) {
        ...
        boolean recreateInstance = false;
        
        SendMailCommand instance = null;
        if(getPropertyAsBoolean(REUSE_TRANSPORT) == false) {
            instance = new SendMailCommand();
            recreateInstance = true;
        } else {
            instance = localInstance.get();
            if(instance == null) {
                instance = new SendMailCommand();
        ...
        if(recreateInstance == true) {
            instance.setSmtpServer(getPropertyAsString(ExtSMTPSampler.SERVER));
            instance.setSmtpPort(getPropertyAsString(ExtSMTPSampler.SERVER_PORT));
        ...
        //Extensions
            instance.setConnectionTimeout(getPropertyAsLong(CONNECTION_TIMEOUT));
            instance.setSocketTimeout(getPropertyAsLong(SOCKET_TIMEOUT));
            instance.setQuitWait(getPropertyAsBoolean(QUIT_WAIT));
        }
        ...
        final String receiverTo = getPropertyAsString(ExtSMTPSampler.RECEIVER_TO).trim();
        final String receiverCC = getPropertyAsString(ExtSMTPSampler.RECEIVER_CC).trim();
        final String receiverBcc = getPropertyAsString(ExtSMTPSampler.RECEIVER_BCC).trim();
        final String replyTo = getPropertyAsString(ExtSMTPSampler.MAIL_REPLYTO).trim();
        ...
        try {
            ...
            instance.setHeaderFields((CollectionProperty)getProperty(ExtSMTPSampler.HEADER_FIELDS));
        ...
        try {
            ...
            if(getPropertyAsBoolean(REUSE_TRANSPORT) == true) {
                instance.reuseExecute(message);
            } else {
                instance.execute(message);
            }
        ...
        if(getPropertyAsBoolean(ExtSMTPSampler.RECONNECT) == true && res.getResponseCode().equals("500")) {
                try {
                    instance.reconnect();
                    log.info(Thread.currentThread().getName() +  " instance reconnected");
                } catch (MessagingException mex) {
                    res.setResponseMessage("Reconnect Exception: " + mex.toString());
                }
        }
    ...
    public boolean interrupt() {
        SendMailCommand instance = localInstance.get();
        if(instance != null) {
            try {
                instance.destroyTransport();
            } catch (MessagingException e) {
                log.error("Can't destroy transport", e);
            }
        }
